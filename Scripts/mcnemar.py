...
def is_correct(pred_bbox, gt_bbox, iou_thr=0.50):
    if pred_bbox is None or gt_bbox is None:
        return False
    return iou(pred_bbox, gt_bbox) >= iou_thr

# --- Load CSVs ---
model_a_df = pd.read_csv('/home/akore/model_a_predictions_processed.csv')
model_b_df = pd.read_csv('/home/akore/model_b_gt_comparison_OCN2.csv')

# --- Merge on Filename ---
merged = pd.merge(model_a_df, model_b_df, on='Filename', suffixes=('_a', '_b'))
...
for idx, row in merged.iterrows():
    bbox_a = parse_bbox(row['bbox_modelA'])
    bbox_b = parse_bbox(row['bbox_modelB'])
    bbox_gt = parse_bbox(row['bbox_gt'])

    if bbox_a is None or bbox_b is None or bbox_gt is None:
        continue  # skip rows with invalid bbox

    correct_a = is_correct(bbox_a, bbox_gt)
    correct_b = is_correct(bbox_b, bbox_gt)
    results.append((correct_a, correct_b))
