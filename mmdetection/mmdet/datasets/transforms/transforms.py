...
from mmcv.transforms import Normalize as MMCV_Normalize
...
class Resize(MMCV_Resize):
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        #print(f"Received from LoadAnnotations in resize: {results}")
        if self.scale:
...
class Pad(MMCV_Pad):
    ...
    def _pad_masks(self, results: dict) -> None:
        #print(f"Received from resize in pad: {results}")
...
@TRANSFORMS.register_module()
class ResizeSequences(BaseTransform):
    ...
    def transform(self, results):
        ...
        #print(f"Results resizesequences: {results}")
        # Get dimensions from the stacked image
        h, w = results['img_shape'][:2]
        
        # Calculate scale factor once
        if self.keep_ratio:
            new_h, new_w = mmcv.imrescale(
                np.zeros((h,w)), self.scale, return_scale=False).shape[:2]
            scale_factor = min(new_h/h, new_w/w)
        else:
            scale_factor = self.scale[0] / w

        # Resize the stacked frames
        results['img'] = mmcv.imrescale(
            results['img'], 
            scale_factor, 
            backend='cv2'
        )
        ...
        #    if 'gt_bboxes' in frame_data and len(frame_data['gt_bboxes']) > 0:
        #        print(f"Sample transformed bbox values: {frame_data['gt_bboxes'][0]}")
        #    else:
        #        print("No bboxes in first frame")
        return results

@TRANSFORMS.register_module()
class PadSequences(BaseTransform):
    ...
    def transform(self, results):
        ...
        padded_img = np.pad(
            img,
            pad_width=((0, pad_h), (0, pad_w), (0, 0)),  # Only pad H,W
            mode='constant',
            constant_values=self.pad_val
        )
        
        results['img'] = padded_img
        results['pad_shape'] = padded_img.shape[:2]
...
@TRANSFORMS.register_module()
class FormatTemporalInput(BaseTransform):
    """Prepare temporal input for packing and convert to tensor."""

    def transform(self, results):
        if 'img' in results:
            img = results['img']
            if img.ndim == 4 and img.shape[3] in [1, 3]:  # Has channels
...
@TRANSFORMS.register_module()
class MultiFrameAddRGBChannel(object):
    """Convert multi-frame grayscale image to multi-frame RGB image.
    """
    ...
    def __call__(self, results):
        ...
        img = np.repeat(img, 3, axis=-1)    # (H,W,T,3)
        
        # Reorder to (T,H,W,3)
        img = np.transpose(img, (2, 0, 1, 3))
        results['img'] = img.astype(np.float32)  # Ensure float32
        #print(f"Transform {self.__class__.__name__} output shape: {results['img'].shape}")
        return results
...
@TRANSFORMS.register_module()
class TemporalNormalize(BaseTransform):
...
class TemporalRandomFlip(BaseTransform):
    ...
    def transform(self, results):
        ...
        direction = np.random.choice(self.direction) if isinstance(self.direction, list) else self.direction
        ...
        if 'gt_bboxes' in results:
            ...
            if isinstance(bboxes, HorizontalBoxes):
                ...
                if direction == 'horizontal':
                    flipped_bboxes[..., [0, 2]] = w - bboxes[..., [2, 0]]
                elif direction == 'vertical':
                    flipped_bboxes[..., [1, 3]] = h - bboxes[..., [3, 1]]
                results['gt_bboxes'] = flipped_bboxes
...
@TRANSFORMS.register_module()
@avoid_cache_randomness
class TemporalAlbu(BaseTransform):
    """Albumentation augmentation for temporal sequences.
    
    Applies the same Albumentation transform to all frames in a sequence.
    """
    ...
    def __init__(self,
        ...
        if bbox_params is not None:
            self.bbox_params.update(bbox_params)
            
        self.aug = Compose([self.albu_builder(t) for t in self.transforms],
                         bbox_params=self.bbox_params)

    def albu_builder(self, cfg: dict) -> albumentations:
        """Same as original Albu implementation"""
        assert isinstance(cfg, dict) and 'type' in cfg
        args = cfg.copy()
        obj_type = args.pop('type')
        if is_str(obj_type):
    ...
    @autocast_box_type()
    def transform(self, results: dict) -> Union[dict, None]:
        """Apply same Albumentation transform to all frames in sequence."""
        # Check if there are any bboxes
        ...
        # Store transform parameters from first frame
        transform_params = None
        img = results['img']  # (T,C,H,W)
        bboxes = results['gt_bboxes'].tensor.numpy() if hasattr(results['gt_bboxes'], 'tensor') else results['gt_bboxes']
        labels = results['gt_bboxes_labels']
        
        # Apply to first frame to get params
        aug_first = self.aug(
        ...
        processed_frames = []
        for frame in img:
            transformed = self.aug.apply(
                img=frame.transpose(1,2,0),
                **{k:v for k,v in aug_first.items() if k != 'image'})
            processed_frames.append(transformed['image'].transpose(2,0,1))
        
        results['img'] = np.stack(processed_frames, axis=0)
        results['gt_bboxes'] = HorizontalBoxes(aug_first['bboxes'])
        results['gt_bboxes_labels'] = np.array(aug_first['labels'])
