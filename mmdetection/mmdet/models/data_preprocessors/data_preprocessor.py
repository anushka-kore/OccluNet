...
@MODELS.register_module()
class TemporalDetDataPreprocessor(DetDataPreprocessor):
    """Optimized version for temporal processing."""
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # Process inputs
        batch_inputs = self._process_inputs(inputs, device)
        
        # Process data samples
        if batch_data_samples is not None:
            data_samples = self._process_data_samples(batch_inputs, data_samples)
        
        # Apply batch augmentations if needed
        if training and self.batch_augments is not None:
            batch_inputs, batch_data_samples = self._apply_batch_augments(
                batch_inputs, batch_data_samples)
        
        return {'inputs': batch_inputs, 'data_samples': batch_data_samples}
    
    def _process_inputs(self, inputs: Union[list, torch.Tensor], device: torch.device) -> torch.Tensor:
        """Process and normalize input sequences."""
        # Convert to tensor and ensure proper shape
        if isinstance(inputs, list):
            # Handle list of temporal sequences
            inputs = [torch.as_tensor(x) for x in inputs]
            max_t = max(x.size(0) for x in inputs)  # Find max sequence length
            padded_inputs = []
            for x in inputs:
                # Pad temporal dimension if sequences have different lengths
                if x.size(0) < max_t:
                    pad_size = max_t - x.size(0)
                    x = F.pad(x, (0,)*6 + (0, pad_size))  # Pad along T dim
                padded_inputs.append(x)
            inputs = torch.stack(padded_inputs, dim=0)  # (B,T,C,H,W)
        else:
            inputs = torch.as_tensor(inputs)
        
        # Ensure proper shape (B,T,C,H,W)
        if inputs.dim() == 4:
            inputs = inputs.unsqueeze(0)  # (T,C,H,W) -> (1,T,C,H,W)
        elif inputs.dim() == 5 and inputs.size(2) in [1, 3]:
            inputs = inputs.permute(0, 1, 4, 2, 3)  # (B,T,H,W,C) -> (B,T,C,H,W)
        
        # Handle grayscale to RGB if needed
        if inputs.size(2) == 1 and self._channel_conversion:
        ...
        if self._enable_normalize:
            mean = torch.as_tensor(self.mean, device=device).view(1, 1, 3, 1, 1)
            std = torch.as_tensor(self.std, device=device).view(1, 1, 3, 1, 1)
        ...
        if pad_h > 0 or pad_w > 0:
            inputs = F.pad(inputs, (0, pad_w, 0, pad_h), value=self.pad_val)
        
        return inputs
    
    def _process_data_samples(self, inputs, data_samples):
        """Update data samples with temporal-aware metadata."""
        B, T, C, H, W = inputs.shape
        
        for sample in data_samples:
            # Set spatial padding info
            sample.set_metainfo({
                'batch_input_shape': (H, W),
                'pad_shape': (H, W),
                'temporal_length': T  # Store original sequence length
            })
            
            # Convert bboxes to tensor if needed
            if self.boxtype2tensor:
                if 'gt_instances' in sample:
                    sample.gt_instances.bboxes = sample.gt_instances.bboxes.tensor
                if 'pred_instances' in sample:
                    sample.pred_instances.bboxes = sample.pred_instances.bboxes.tensor
        
        print(f"TemporalDetDataPreprocessor: {data_samples}")
        return data_samples
'''
@MODELS.register_module()
class TemporalDetDataPreprocessor(DetDataPreprocessor):
    """Fixed version that handles device consistency."""
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        batch_inputs = data['inputs']  # List of sequences
        batch_data_samples = data.get('data_samples', None)
        
        # Convert all sequences to tensors on target device
        processed_sequences = []
        batch_pad_shapes = []
        
        for seq_idx, sequence in enumerate(batch_inputs):
            # Ensure sequence is tensor and on correct device
            ...
            #if sequence.dim() == 4 and sequence.shape[3] in [1, 3]:  # (T, H, W, C)
            ...
            if sequence.shape[-1]==3:  
                ...
                sequence = sequence.permute(0, 3, 1, 2).repeat(1, 3, 1, 1)  # (T,1,H,W) -> (T,3,H,W)
            ...
            for frame in sequence:
                # Calculate pad shape
                pad_h = int(np.ceil(frame.shape[-2] / self.pad_size_divisor)) * self.pad_size_divisor
                pad_w = int(np.ceil(frame.shape[-1] / self.pad_size_divisor)) * self.pad_size_divisor
                seq_pad_shapes.append((pad_h, pad_w))
                
                # Normalize on correct device
                frame = frame.float()
                if self._enable_normalize:
                    # Ensure mean/std are on same device as frame
                    mean = torch.tensor(self.mean, device=frame.device)
                    std = torch.tensor(self.std, device=frame.device)
                    frame = (frame - mean) / std
                frames.append(frame)
            
            processed_sequences.append(torch.stack(frames))
            batch_pad_shapes.append(seq_pad_shapes)
        
        # Step 3: Pad and stack sequences
        if processed_sequences:
            max_t = max(s.shape[0] for s in processed_sequences)
            C, H, W = processed_sequences[0].shape[1:]
            
            padded_sequences = []
            for seq in processed_sequences:
                if seq.shape[0] < max_t:
                    padding = torch.zeros((max_t - seq.shape[0], C, H, W),
            ...
            inputs = torch.empty(0, 5, device=device)
        
        # Step 4: Process data samples
        if batch_data_samples:
            for b, data_sample in enumerate(batch_data_samples):
                if b < len(batch_pad_shapes):
                    data_sample.set_metainfo({
                        'batch_input_shape': inputs.shape[-2:],
                        'temporal_pad_shapes': batch_pad_shapes[b],
                        'pad_shape': batch_pad_shapes[b][0] if batch_pad_shapes[b] else inputs.shape[-2:]
                    })
                    ...
                    if self.pad_mask and training:
                        self.pad_gt_masks([data_sample])
                    
                    if self.pad_seg and training:
                        self.pad_gt_sem_seg([data_sample])
        
        # Step 5: Apply batch augmentations
        if training and self.batch_augments is not None:
            inputs, batch_data_samples = self._apply_batch_augments(
                inputs, batch_data_samples)
        
        return {'inputs': inputs, 'data_samples': batch_data_samples}

    def _apply_batch_augments(self, inputs, data_samples):
        """Helper method to apply batch augmentations."""
        B, T = inputs.shape[:2]
        ...
        if data_samples:
            for sample in data_samples:
                flat_samples.extend([sample] * T)
        
        # Apply augmentations
        for batch_aug in self.batch_augments:
            batch_aug = batch_aug.to(device)
            flat_inputs, flat_samples = batch_aug(flat_inputs, flat_samples)
        
        # Reshape back
        inputs = flat_inputs.view(B, T, *flat_inputs.shape[1:])
        return inputs, data_samples
'''