...
class CocoMetric(BaseMetric):
        ...
        for idx, gt_dict in enumerate(gt_dicts):
            ...
            for ann in gt_dict['anns']:
                ...
                annotation = dict(
                    ...
                    image_id=img_id,
                    bbox=coco_bbox,
                    iscrowd=ann.get('ignore_flags', 0),
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
          ...
          # Convert the instances to proper annotation format    
          instances = data_sample['instances']
                anns = []
                for i in range(len(instances['labels'])):
                    ann = {
                        'labels': instances['labels'][i].cpu().numpy(),
                        'bboxes': instances['bboxes'][i].cpu().numpy(),
                        'ignore_flags': instances['ignore_flags'][i].cpu().numpy()
                    }
                    anns.append(ann)
                gt['anns'] = anns
